...
{
    //устанавливается позиция первой вершины
    CircleShape circle(80.f / pow(1.1, size));
    circle.setOrigin(circle.getRadius(), circle.getRadius());
    circle.setFillColor(Color(0, 0, 193));
    ...
    int current = 0;
    int temp = size;
    vector<pair<float, float>> v_coords(numVertices); //вектор для хранения координат вершин
                                                      //заполнение
    ...
    for (size_t i = 0; i < numVertices; ++i)
    {
        ...
        {
            RectangleShape line;                      //устанавливаются значения для рёбер графа
            line.setFillColor(Color(255, 0, 128));
            if (i != j && matrix[i][j] != 100000)
                if (matrix[i][j] == matrix[j][i])
                {
                    Text txt2(to_string(matrix[i][j]), fnt, 35 / pow(1.1, size)); //значения для вершин графа из матрицы смежности
                    txt2.setOrigin(txt2.getLocalBounds().width / 2.f, txt2.getLocalBounds().height);
                    txt2.setFillColor(Color(0, 0, 0));
                    txt2.setOutlineThickness(4);
                    txt2.setOutlineColor(Color(113, 253, 204));
    ...
    //отрисовка кнопок
    RectangleShape rect;
    rect.setFillColor(Color(255, 0, 128));
    ...
    for (size_t i = 0; i < 5; i++)
        if (this->button_coords[i].x <= ButtonPressedCoords.x && this->button_coords[i].x + 235 >= ButtonPressedCoords.x && this->button_coords[i].y <= ButtonPressedCoords.y && this->button_coords[i].y + 50 >= ButtonPressedCoords.y)
            ...
            case 4:
            {
                ...
                window.create(VideoMode(280, size*32), "DeleteVertex", Style::None);
                window.setPosition(Vector2i(w.getView().getCenter().x - window.getView().getCenter().x + 500, 40));
                        ...
                        if (event.type == sf::Event::TextEntered)
                        {
                            if (event.text.unicode == '\r') //enter
                            {
                                ...
                                else
                                {
                                    ...
                                    {
                                        wayS += " -> " + to_string(currentCity) + '\n' + to_string(currentCity);
                                        currentCity = way[currentCity];
                                    }
                                    wayS += " -> " + to_string(startCity);

                                    txt4.setPosition(txt2.getLocalBounds().width + 10, 87);
                                    txt4.setString(wayS);
